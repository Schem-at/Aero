...
function requestChunks(): void {
  ...
  if (needed.length > 0) {
    ...
    // No new chunks needed, but we still need to send batch finish
    // (the position handler already sent Chunk Batch Start)
...
async function registerRoom(room: string): Promise<void> {
  if (!transport) return;
